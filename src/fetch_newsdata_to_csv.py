...
def _load_existing_csv(output_path: Path, columns: list[str]) -> tuple[list[dict[str, str]], dict[str, Any]]:
...
def _merge_news_rows(
	...
	seen_keys = {
		key
		for key in (_row_unique_key(row) for row in existing_rows)
		if key
	}
	appended_count = 0
	...
	if not isinstance(existing_metadata.get("request_history"), list):
		if isinstance(existing_metadata.get("query_parameters"), dict):
			legacy_entry = {
				...
				"rows_fetched_in_request": existing_metadata.get("query_results_summary", {}).get("rows_fetched"),
